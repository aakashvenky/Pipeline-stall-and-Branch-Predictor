...
void pipe_cycle_WB(Pipeline *p)
{
    ...
    {  //Counter variable based on number of stalls decremented at WB stage
...
void pipe_cycle_ID(Pipeline *p)
{
...

    {
        ...
        if (p->pipe_latch[MA_LATCH][i].trace_rec.dest_needed == ((p->pipe_latch[ID_LATCH][i].trace_rec.src1_needed) or (p->pipe_latch[ID_LATCH][i].trace_rec.src2_needed))
        {
            if (p->pipe_latch[MA_LATCH][i].trace_rec.dest_reg) == ((p->pipe_latch[ID_LATCH][i].trace_rec.src1_reg) or (p->pipe_latch[ID_LATCH][i].trace_rec.src2_reg))
            {
                p->pipe_latch[ID_LATCH][i].valid = false;
            }
        }
        if (p->pipe_latch[IE_LATCH][i].trace_rec.dest_needed == ((p->pipe_latch[ID_LATCH][i].trace_rec.src1_needed) or (p->pipe_latch[ID_LATCH][i].trace_rec.src2_needed))
        {
            if (p->pipe_latch[IE_LATCH][i].trace_rec.dest_reg) == ((p->pipe_latch[ID_LATCH][i].trace_rec.src1_reg) or (p->pipe_latch[ID_LATCH][i].trace_rec.src2_reg))
            {
                p->pipe_latch[ID_LATCH][i].valid = false;
            }
        }
        
        if (!p->pipe_latch[ID_LATCH[i]].stall == false) and ( )
        {
            p->pipe_latch[ID_LATCH][i] = p->pipe_latch[IF_LATCH][i];
        }
